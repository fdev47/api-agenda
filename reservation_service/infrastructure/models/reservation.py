...
class ReservationModel(Base):
    ...
    def to_domain(self) -> 'Reservation':
        ...
        try:
            ...
            # Convertir strings de UUID de vuelta a UUIDs
            customer_data_dict = self.customer_data.copy()
            if customer_data_dict.get('id'):
                customer_data_dict['id'] = uuid.UUID(customer_data_dict['id'])
            ...
            logger.error(f"❌ Error en ReservationModel.to_domain(): {str(e)}", exc_info=True)
            raise
    ...
    def from_domain(cls, reservation: 'Reservation') -> 'ReservationModel':
        ...
        try:
            ...
            logger.error(f"❌ Error en ReservationModel.from_domain(): {str(e)}", exc_info=True)
            raise
