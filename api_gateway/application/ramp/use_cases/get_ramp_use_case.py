"""
Caso de uso para obtener rampa en API Gateway
"""
import logging
...
class GetRampUseCase:
    """Caso de uso para obtener una rampa"""
    ...
    async def execute(self, ramp_id: int, access_token: str = "") -> RampResponse:
        """
        Ejecutar el caso de uso para obtener una rampa
        
        Args:
            ramp_id: ID de la rampa a obtener
            access_token: Token de acceso para autenticación
            
        Returns:
            RampResponse: Rampa obtenida
            
        Raises:
            HTTPError: Si hay errores en el servicio de ubicación
        """
        logger.info(f"🚀 Ejecutando GetRampUseCase para rampa {ramp_id}")
        
        # Usar context manager para el cliente HTTP
        async with self.location_client as location_client:
            # Llamar al servicio de ubicación para obtener la rampa
            logger.info(f"📞 Llamando al location_service para obtener rampa {ramp_id}")
            ...
            logger.info(f"✅ Rampa {ramp_id} obtenida exitosamente")
